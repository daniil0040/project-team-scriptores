import{g as R,b as N,c as g,a as l,f as b,n as $}from"./scroll-up-51cac475.js";const x=document.querySelector(".fav-container-js");x.addEventListener("click",q);function q(e){if(!e.target.classList.contains("btn-see-recipe-js"))return;const n=e.target.closest(".btn-see-recipe-js").dataset.id;V(n)}async function V(e){try{const t=await R(e);_(t)}catch(t){console.log(t)}}function _(e){const t=N.create(O(e));t.show();const n=document.querySelector(".js-modal-video"),a=document.querySelector(".js-modal-img");W(e.youtube,n,a);const i=document.querySelector(".js-favorite-btn");(JSON.parse(localStorage.getItem(g.LS_RECIPES))??[]).find(({_id:c})=>c===e._id)?i.textContent="Remove":i.textContent="Add to favorite",i.addEventListener("click",()=>{i.textContent=Y(e);const c=JSON.parse(localStorage.getItem(g.LS_RECIPES));x.innerHTML=l(c),b(),m(),t.close(),location.reload(!0)}),document.querySelector(".js-modal-close").addEventListener("click",()=>t.close()),document.addEventListener("keydown",c=>{c.code==="Escape"&&(t.close(),document.removeEventListener("keydown"))})}function O(e){let t;const{thumb:n,title:a,rating:i,time:o,instructions:u,ingredients:d,tags:c,youtube:w}=e,j=i.toFixed(1),T=d.map(({name:k,measure:P})=>`<li class="ing-item">
    ...
    </li>`,a=Math.round(e);let i=[];for(let o=0;o<5;o++)o+1>a?i.push(n):i.push(t);return i.join("")}function U(e){let t=e,n=t.indexOf("=");if(n!==-1)return t.substring(n+1)}function W(e,t,n){e||(t.classList.add("is-none"),n.classList.remove("is-none"))}function Y(e){const t=JSON.parse(localStorage.getItem(g.LS_RECIPES))??[],n=t.find(({_id:i})=>i===e._id),a=t.findIndex(({_id:i})=>i===e._id);if(n){t.splice(a,1),localStorage.setItem(g.LS_RECIPES,JSON.stringify(t));let i="Add to favorite";return $.Notify.warning("The recipe removed from favorites"),i}else{t.push(e),localStorage.setItem(g.LS_RECIPES,JSON.stringify(t));let i="Remove";return $.Notify.success("The recipe is added to favorites"),i}}let S,s=JSON.parse(localStorage.getItem(g.LS_RECIPES))??[],p,C=1;const A=document.querySelector(".fav-empty"),r=document.querySelector(".fav-container"),M=document.querySelector(".fav-categories"),v=document.querySelector(".fav-static");G();function G(){if(s.length)A.classList.add("is-none"),F(),I(),et();else return}function I(){const e="category",t=s.map(i=>i[e]),a=[...new Set(t)];a.unshift("All categories"),K(a)}function K(e){M.innerHTML=D(e),document.querySelectorAll(".js-category-btn").forEach(n=>{n.addEventListener("click",function(a){const i=a.currentTarget.getAttribute("data-button");i==="All categories"?F():z(i)})})}function z(e){const t=s.filter(n=>n.category===e);document.getElementById("paginationFAV").innerHTML="",r.innerHTML=l(t),b(),m()}function D(e){return e.map(t=>`<button class="category-btn js-category-btn" data-button="${t}">${t}</button>`).join("")}function F(){s.length>12&&window.innerWidth>767?(p=s.slice(0,12),r.innerHTML=l(p),y(s)):s.length>9&&window.innerWidth<=767?(p=s.slice(0,9),r.innerHTML=l(p),y(s)):(document.getElementById("paginationFAV").innerHTML="",r.innerHTML=l(s)),b(),m(),v.classList.contains("fav-phantom")&&(v.classList.remove("fav-phantom"),r.classList.remove("fav-style-reset"))}function y(e,t=C){const n=Math.ceil(e.length/12),a=t,i=Z(n,a),o=Number(i)+2;if(n===1)return document.getElementById("paginationFAV").innerHTML="";const u=Q(a),d=tt(i,o,n,t),c=X(a,n),w=`<div class="pagination-bar">
    <div class="pag-btn-block">${u}</div>
    <div class="pag-btn-block">${d}</div>
    <div class="pag-btn-block">${c}</div>
  </div>`;document.getElementById("paginationFAV").innerHTML=w,b(),m()}function Q(e){const t=e-1;let n="pag-btn-number";return Number(e)===1&&(n=""),`<button
      id="pag-btn-start"
    ...
    >
      <span class="icon-wrap left-single"></span>
        ...
        >
          ...
        </button>`),i}let L=0,h=12,f;document.addEventListener("click",async e=>{const t=e.target.closest(".pag-btn-number");if(p=E(t),t){S=t;const n=t.getAttribute("page-number"),a=l(p);r.innerHTML=a,b(),m(),y(s,n)}});function E(e){return e===1?s.slice(0,12):(e&&(f=e.getAttribute("page-number")),s.length>12&&window.innerWidth>767?(L=12*f-12,h=12*f,s.slice(L,h)):(L=9*f-9,h=9*f,s.slice(L,h)))}function et(){r.addEventListener("click",e=>{if(!e.target.classList.contains("js-add"))return;const t=e.target.dataset.id,n=s.findIndex(({_id:a})=>a===t);s.splice(n,1),localStorage.setItem(g.LS_RECIPES,JSON.stringify(s)),nt(S??1),b(),m(),I(),s.length||(A.classList.remove("is-none"),v.classList.add("fav-phantom"),r.classList.add("fav-style-reset"),M.innerHTML="")})}function m(){document.querySelectorAll(".js-add").forEach(t=>t.classList.add("liked"))}function nt(e){s.length>12&&window.innerWidth>767?(S&&(C=S.getAttribute("page-number")),r.innerHTML=l(E(e)),y(s)):s.length>9&&window.innerWidth<=767?(r.innerHTML=l(E(e)),y(s)):(document.getElementById("paginationFAV").innerHTML="",r.innerHTML=l(s)),b(),m(),v.classList.contains("fav-phantom")&&(v.classList.remove("fav-phantom"),r.classList.remove("fav-style-reset"))}
