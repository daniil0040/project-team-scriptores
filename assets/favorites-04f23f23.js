import{g as R,b as N,c as g,a as l,f as L,n as x}from"./scroll-up-2d20b6e5.js";const $=document.querySelector(".fav-container-js");$.addEventListener("click",q);function q(e){if(!e.target.classList.contains("btn-see-recipe-js"))return;const n=e.target.closest(".btn-see-recipe-js").dataset.id;V(n)}async function V(e){try{const t=await R(e);_(t)}catch(t){console.log(t)}}function _(e){const t=N.create(O(e));t.show();const n=document.querySelector(".js-modal-video"),i=document.querySelector(".js-modal-img");W(e.youtube,n,i);const s=document.querySelector(".js-favorite-btn");(JSON.parse(localStorage.getItem(g.LS_RECIPES))??[]).find(({_id:c})=>c===e._id)?s.textContent="Remove":s.textContent="Add to favorite",s.addEventListener("click",()=>{s.textContent=Y(e);const c=JSON.parse(localStorage.getItem(g.LS_RECIPES));$.innerHTML=l(c),L(),S(),t.close(),location.reload(!0)}),document.querySelector(".js-modal-close").addEventListener("click",()=>t.close()),document.addEventListener("keydown",c=>{c.code==="Escape"&&(t.close(),document.removeEventListener("keydown"))})}function O(e){let t;const{thumb:n,title:i,rating:s,time:o,instructions:u,ingredients:d,tags:c,youtube:w}=e,j=s.toFixed(1),T=d.map(({name:k,measure:P})=>`<li class="ing-item">
    <p class="ing-name">${k}</p>
    <p class="ing-measure">${P}</p>
    </li>`).join("");let H=c.map(k=>`<li class="tags-item">
    <p class="tags-name">#${k}</p>
    </li>`).join(""),B=J(s);return t=`<div class="modal-box categories-block-modal"><div class="img-title-box"><iframe class="modal-img js-modal-video"  width="295" height="295" src="https://www.youtube.com/embed/${U(w)}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
    ...
    >
      <svg class="pag-btn-left-icon" width="20" height="20">
        ...
        >
          ...
        </button>`),s}let v=0,h=12,m;document.addEventListener("click",async e=>{const t=e.target.closest(".pag-btn-number");if(b=E(t),t){y=t;const n=t.getAttribute("page-number"),i=l(b);r.innerHTML=i,f(a,n)}});function E(e){return e===1?a.slice(0,12):(e&&(m=e.getAttribute("page-number")),a.length>12&&window.innerWidth>767?(v=12*m-12,h=12*m,a.slice(v,h)):(v=9*m-9,h=9*m,a.slice(v,h)))}function et(){r.addEventListener("click",e=>{if(!e.target.classList.contains("js-add"))return;const t=e.target.dataset.id,n=a.findIndex(({_id:i})=>i===t);a.splice(n,1),localStorage.setItem(g.LS_RECIPES,JSON.stringify(a)),nt(y??1),I(),a.length||(A.classList.remove("is-none"),p.classList.add("fav-phantom"),r.classList.add("fav-style-reset"),M.innerHTML="")})}function S(){document.querySelectorAll(".js-add").forEach(t=>t.classList.add("liked"))}function nt(e){a.length>12&&window.innerWidth>767?(y&&(C=y.getAttribute("page-number")),r.innerHTML=l(E(e)),f(a)):a.length>9&&window.innerWidth<=767?(r.innerHTML=l(E(e)),f(a)):(document.getElementById("paginationFAV").innerHTML="",r.innerHTML=l(a)),p.classList.contains("fav-phantom")&&(p.classList.remove("fav-phantom"),r.classList.remove("fav-style-reset"))}
